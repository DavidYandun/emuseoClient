...
export class IdentificationService {
  ...
  getCollectionAprobado(): Observable<any> {
    return this.httpClient.get('/api/identifications/collectionaprobados').pipe(
      catchError(this.errorHandler));
  }
  getCollectionPDF(): Observable<any> {
    return this.httpClient.get('/api/identifications/collectionPDF').pipe(
      catchError(this.errorHandler));
  }

  getReinos(kingdom): Observable<any> {
    return this.httpClient.get('/api/identifications/collection/' + kingdom).pipe(
      catchError(this.errorHandler));;
  }
  
  getReinosAprobado(kingdom): Observable<any> {
    return this.httpClient.get('/api/identifications/collectionaprobados/' + kingdom).pipe(
      catchError(this.errorHandler));;
  }
  getVerification(verificationstatus): Observable<any> {
    return this.httpClient.get('/api/identifications/verification/' + verificationstatus).pipe(
      catchError(this.errorHandler));;
  }
